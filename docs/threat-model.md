...
| An agent's token relayed between hub replicas goes only to a replica | Implemented: a capacity report is relayed with the agent's own `Authorization` header, dialled straight to each address the peers Service resolves, through no proxy and following no redirect. The relay's log lines name the peer and the cell, never a header, and a test relays a sentinel token to failing peers and searches the log for it ([ADR-014](architecture.md#adr-014-a-replica-relays-each-capacity-report-to-the-others)) |
...
| Relaying capacity between replicas multiplies only accepted reports | Implemented: a report is relayed once to each other replica after it passed authentication, the registration check and the reporter check, never after a refusal, and never again by a replica that received it as a relay. Each relay is bounded by a timeout |
...
| A report relayed by another hub replica is checked like the agent's own | Implemented: every replica authenticates the agent's token and checks `spec.reporter` itself. The relay header only stops a report being relayed again, and a test sends it from the wrong reporter and is refused |
...
| A relayed agent token can reach a pod that is no longer a hub replica | Replicas are looked up on every report, so this needs a departed replica's address reused by another pod within one DNS answer's lifetime, seconds on a default cluster. The token reports capacity for one cell until it expires, which is the compromised-agent risk above | If the hub serves TLS itself, so a relay can check whom it reached |
