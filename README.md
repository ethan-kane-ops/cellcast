...
v0.5.0. The placement path, the broker, the audit trail, metrics, multi-replica HA, both charts,
...
```bash
cosign verify ghcr.io/ethan-kane-ops/cellcast-hub:v0.5.0 \
  --certificate-identity https://github.com/ethan-kane-ops/cellcast/.github/workflows/release.yml@refs/tags/v0.5.0 \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

The same command covers the agent image, the client image and both charts
(`ghcr.io/ethan-kane-ops/charts/cellcast:0.5.0`). For the client archives, verify the checksum file
...
```bash
docker buildx imagetools inspect ghcr.io/ethan-kane-ops/cellcast-hub:v0.5.0 --format '{{ json .SBOM }}'
```
